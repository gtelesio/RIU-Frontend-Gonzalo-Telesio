...
export class HeroFormPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      this.isEdit = true;
      this.repo.getById(id).subscribe(hero => this.hero = hero || null);
    }
  }

  onSave(heroData: Omit<SuperHero, 'id'>) {
    if (this.isEdit && this.hero) {
      this.repo.update({ ...this.hero, ...heroData }).subscribe(() => this.router.navigate(['/superheroes']));
    } else {
      this.repo.create(heroData).subscribe(() => this.router.navigate(['/superheroes']));
    }
  }
